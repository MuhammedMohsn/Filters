...
function show_products() {
    if (filtered_products.length < 1) { products_section.innerHTML = `<h4>no items here</h4>` }
        ...
        products_section.innerHTML = filtered_products.map(product => {
         ...
         <h6 class="price text-center"> ${product.price}</h6>
         </div>`
        }).join('')
    }
}
...
filter_btnss.forEach(btn => {
    btn.addEventListener("click", (e) => {
        ...
        if (id == "all") { filtered_products = [...data]; }
        else {
            filtered_products = filtered_products.filter(product => { return product.company == id })
